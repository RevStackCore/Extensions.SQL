...
namespace RevStackCore.Extensions.SQL
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        private static List<PropertyInfo> ExplicitKeyPropertiesCache(Type type)
        {
            ...
            var primaryExplicitKeyProperties = TypePropertiesCache(type).Where(p => p.GetCustomAttributes(true).Any(a => a is PrimaryKeyAttribute && ((PrimaryKeyAttribute)a).ExplicitKey) == true).ToList();
            if (primaryExplicitKeyProperties.Any())
            {
                explicitKeyProperties.AddRange(primaryExplicitKeyProperties);
            }
        ...
        public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < allPropertiesExceptKeyAndComputed.Count; i++)
            {
                ...
                sbParameterList.AppendFormat("@{0}", fName);
                if (i < allPropertiesExceptKeyAndComputed.Count - 1)
            ...
            {
                string _name = name;
                string _columnList = sbColumnList.ToString();
                string _parameterList = sbParameterList.ToString();
                var _entityToInsert = entityToInsert;
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < nonIdProps.Count; i++)
            {
                ...
                adapter.AppendColumnNameEqualsValue(sb, property.ToPropertyName(), property.Name);  //fix for issue #336
