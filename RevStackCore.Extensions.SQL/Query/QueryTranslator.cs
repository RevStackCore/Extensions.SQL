    ...
    internal class QueryTranslator : ExpressionVisitorBase
    {
        ...
        protected override Expression VisitMethodCall(MethodCallExpression m)
        {
            ...
            if (m.Method.DeclaringType == typeof(Queryable) ||
                ...
                m.Method.Name == "StartsWith" || m.Method.Name=="EndsWith" || m.Method.Name=="ToLower" || m.Method.Name=="ToUpper")
            {
                ...
                else if (m.Method.Name == "Single" || m.Method.Name == "SingleOrDefault")
                {
                    switch(_languageType)
                    {
                        case SQLLanguageType.SQLServer:
                            this.Visit(m.Arguments[0]);
                            if(sb.ToString().IndexOf(" TOP ", StringComparison.Ordinal) < 0)
                            {
                                string[] parts = sb.ToString().Split(new string[] {"SELECT "}, StringSplitOptions.None);
                ...
                else if (m.Method.Name == "First" || m.Method.Name == "FirstOrDefault")
                {
                    switch (_languageType)
                    {
                        case SQLLanguageType.SQLServer:
        ...
        protected override Expression VisitMemberAccess(MemberExpression m)
        {
            ...
            else if (m.Expression != null && m.Expression.NodeType == ExpressionType.Constant || m.Expression.NodeType == ExpressionType.MemberAccess)
            {
                ...
                if (v == null)
                    v = "null";
                if (m.Member.Name == "Contains" && v != null && v is string)
                {
                    v = "\'%" + v.ToString() + "%\'";
                }
